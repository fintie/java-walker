...
public class WalkerHandler {
	...
	@RequestMapping(value="/client/publish", method=RequestMethod.POST)
...
//		jobService.save(job);		
	...
	public String accept(@PathVariable("id") Integer id, @RequestParam(value="wid") Integer wid){
		if(jobService.getJob(id).getStatus().trim().toLowerCase().equals("pending")){
			...
			for(Walker w: walkers){
				...
				if(w.getRole().trim().toLowerCase().equals("walker")){
					String link = "<a href = http://localhost:8080/" + "Walker/walker/take/" + id + "/" + w.getId() + ">" + "GET NEW JOB" + "</a>"; 
	...
	@RequestMapping(value ="/walker/take/{id}/{wid}", method=RequestMethod.GET )
	public String take(@PathVariable("id") Integer id, @PathVariable("wid") Integer wid){
